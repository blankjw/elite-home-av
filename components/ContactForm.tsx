...
function ContactFormInner() {
  ...
  const fieldClass =
    "w-full bg-transparent border-b border-[#333333] text-white placeholder-[#4B5563] px-0 py-3 text-sm focus:outline-none focus:border-[#AAB6C5] transition-colors"
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-[#31445A] pt-8">
      ...
      {status === "error" ? (
        <p className="mb-6 text-sm text-red-400">Your message could not be sent. Please try again or call Call {CONTACT.phone}.</p>
      ) : null}
      <div className="space-y-6">
        <fieldset className="text-sm text-[#C3CCD7]">
          <legend className="text-xs text-[#9BA7B5] mb-2">What do you need?</legend>
          <label className="mr-6"><input type="radio" name="intent" checked={formData.intent === "new_project"} onChange={() => setFormData((p) => ({ ...p, intent: "new_project" }))} className="mr-2" />New project</label>
          <label><input type="radio" name="intent" checked={formData.intent === "existing_service"} onChange={() => setFormData((p) => ({ ...p, intent: "existing_service" }))} className="mr-2" />Existing system / ELITE Care</label>
        </fieldset>
        <div className="grid sm:grid-cols-2 gap-6">
          <div>
            <label htmlFor="name" className="block text-xs text-[#9BA7B5] mb-2">Name</label>
            <input id="name" required value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} className={fieldClass} />
          </div>
          <div>
            <label htmlFor="phone" className="block text-xs text-[#9BA7B5] mb-2">Phone</label>
            <input id="phone" type="tel" required value={formData.phone} onChange={(e) => setFormData((p) => ({ ...p, phone: e.target.value }))} className={fieldClass} />
          </div>
        </div>
        <div>
          <label htmlFor="email" className="block text-xs text-[#9BA7B5] mb-2">Email (optional)</label>
          <input id="email" type="email" value={formData.email} onChange={(e) => setFormData((p) => ({ ...p, email: e.target.value }))} className={fieldClass} />
        </div>
        <div>
        ...
        <button type="submit" disabled={status === "submitting"} className="text-sm font-medium text-white border-b border-[#AAB6C5] pb-1 hover:text-[#AAB6C5] transition-colors disabled:opacity-50">
